...
public class BodypartsAdapter extends RecyclerView.Adapter<BodypartsAdapter.BodyPartsViewHolder> {
    ...
    public void onBindViewHolder(@NonNull BodyPartsViewHolder holder, int position) {
        ...
        holder.bodyName.setImageResource(bodyPartsModels.get(position).getBodyImg());
        holder.bodyImg.setImageResource(bodyPartsModels.get(position).getBodyName());
        holder.bodyName.setOnClickListener(view -> {
            SoundInterface.playSound(bodyPartsModels.get(position).getBodySound(), view.getContext());
        });
